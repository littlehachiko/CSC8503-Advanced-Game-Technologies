...
public:
	...
	virtual void OnUpdateScene(float dt) override
	{
		...
		if (m_Rotating)
		{
			...
			if (rotating_cuboid1->Physics()->IsCollide())
			{
				rotating_cuboid1->SetColour(whenCol);
			}

			if (orbiting_sphere2->Physics()->IsCollide())
			{
				orbiting_sphere2->SetColour(whenCol);
			}

			if (orbiting_sphere1->Physics()->IsCollide())
			{
				orbiting_sphere1->SetColour(whenCol);
			}
		}
